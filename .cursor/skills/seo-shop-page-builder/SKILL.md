...
| Доверие | с 2010 года, официальный магазин, 15 лет опыта |
...
2. **Collapse wrapper** (see condition below) → `.mm-collapse-content` → `.mm-block`
3. Inside `.mm-block`:
   - **Intro**: H2 («Купить [Категория] в Севастополе — Maxmobiles») + `.mm-intro-text` + `.mm-stat-badge`
   - **Trust strip**: 4 icons — гарантия 1 год, доставка СДЭК, Trade-In, магазин с 2010
   - **Highlights grid**: strictly **3 or 6** `<article>` cards (top models or subcategories)
   - **Advantages**: `.mm-advantages-list` — why buy at Maxmobiles
  - **FAQ**: 4 questions (delivery, guarantee, Trade-In, model choice) in plain HTML (`.mm-faq`) without schema microdata
...
- **Collapse ON** (default): wrap `.mm-block` in `.mm-collapse-wrapper → .mm-collapse-content`, add `.mm-collapse-fade`, `.mm-collapse-trigger`, and the `<script>` block at the end
- **Collapse OFF**: output `.mm-block` directly inside the page, no `.mm-collapse-wrapper`, no `.mm-collapse-fade`, no `<script>` toggle block
...
**E-E-A-T checklist:**
- [ ] **Experience**: упомянут реальный опыт — «с 2010 года», «15 лет», конкретные цифры (клиенты, устройства)
