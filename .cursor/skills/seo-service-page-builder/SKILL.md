...
1. **JSON-LD** — `@graph`: LocalBusiness + WebPage + FAQPage (4 questions from actual content)
2. **Collapse wrapper** → `.mm-collapse-content` → `.mm-block`
3. Inside `.mm-block`:
   ...
   - **Services grid**: strictly 3 or 6 `<article>` cards with `itemprop`
   - **Advantages / Highlight**: `.mm-advantages-list` or `.mm-highlight` block
  - **FAQ**: 4 questions in plain HTML (`.mm-faq`) mapped to FAQPage JSON-LD only (no FAQ microdata in HTML)
   - **CTA**: dark gradient block, 2 buttons (Telegram bot + phone)
4. **collapse `<script>`** at the end
