...
<!--
Источник: https://github.com/TheSmokeDev/geo-skills (MIT License, © 2026 SmokeDev)
Файл skills/geo-schema/SKILL.md скопирован с добавлением предупреждения ниже, специфичного для этого проекта.
LICENSE репозитория лежит рядом, в этой же папке (требование MIT — сохранять copyright notice в копиях).
-->

> ⚠️ **Ограничение для проекта Maxmobiles / Service MM.** Правила этого репозитория (см. `CLAUDE.md`) прямо запрещают:
> - объединение нескольких JSON-LD схем в один блок через `@graph` — конфликтует с CMS-генерируемыми схемами CS-Cart (например, `BreadcrumbList`);
> - схему `FAQPage` — Google отключил rich results для FAQ в августе 2023, FAQ-контент на страницах индексируется через обычный HTML + свойство `speakable`, а не отдельную JSON-LD схему.
>
> **Используй этот скил ТОЛЬКО для аудита существующей разметки** (Step 1 «Detection», Step 2 «Validation», Step 4 «Deprecated/Changed Schemas», Step 5 «sameAs Strategy»). Раздел **Step 6 «JSON-LD Generation»** (включая рекомендацию `@graph`) и генерацию схемы **FAQPage** из Step 3 для страниц maxmobiles.ru не применять — вместо этого генерация JSON-LD для этого проекта делается через существующие `.cursor/skills/seo-meta-builder-v2` / `seo-shop-page-builder-v2`, которые уже соблюдают эти ограничения (каждый тип схемы — отдельный `<script>`, без `@graph`, без FAQPage).
...
> ⚠️ **Для Maxmobiles: не применять.** Проект явно исключает FAQPage-схему (см. предупреждение в начале файла) — FAQ индексируется через HTML + `speakable`, не через отдельный JSON-LD. Используй этот раздел только чтобы распознать и заflagить чужой FAQPage при аудите, не для генерации.
...
> ⚠️ **Для Maxmobiles: не использовать напрямую.** Этот раздел (включая паттерн `@graph`) конфликтует с ограничениями проекта — см. предупреждение в начале файла. Оставлено как справочный материал источника; для генерации новой разметки на maxmobiles.ru использовать `.cursor/skills/seo-meta-builder-v2` / `seo-shop-page-builder-v2`.
