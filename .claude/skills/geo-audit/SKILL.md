...
> ⚠️ **Ограничение для проекта Maxmobiles / Service MM.** Этот скил — orchestration-обёртка, которая делегирует Subagent 5 «Schema & Structured Data» на `geo-schema`. Правила проекта (`CLAUDE.md`) запрещают JSON-LD `@graph` и схему `FAQPage` для HTML-страниц Maxmobiles. Раздел ниже «Subagent 5» и упоминания `FAQPage`/`HowTo` в разделе «Business-Type-Specific Audit Adjustments» — используй только для **аудита** того, что уже есть на странице, не как инструкцию по генерации новой разметки. См. подробности в `.claude/skills/geo-schema/SKILL.md`.
