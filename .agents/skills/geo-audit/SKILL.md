...
Файл skills/geo-audit/SKILL.md скопирован с добавлением предупреждения ниже, специфичного для этого проекта.
LICENSE репозитория лежит рядом, в этой же папке (требование MIT — сохранять copyright notice в копиях).
-->

> ⚠️ **Ограничение для проекта Maxmobiles / Service MM.** Этот скил — orchestration-обёртка, которая делегирует Subagent 5 «Schema & Structured Data» на `geo-schema`. Правила проекта (`AGENTS.md`) запрещают JSON-LD `@graph` и схему `FAQPage` для HTML-страниц Maxmobiles. Раздел ниже «Subagent 5» и упоминания `FAQPage`/`HowTo` в разделе «Business-Type-Specific Audit Adjustments» — используй только для **аудита** того, что уже есть на странице, не как инструкцию по генерации новой разметки. См. подробности в `.Codex/skills/geo-schema/SKILL.md`.

# GEO Audit Orchestration Skill

## Purpose

This skill performs a comprehensive Generative Engine Optimization (GEO) audit of any website. GEO is the practice of optimizing web content so that AI systems (ChatGPT, Codex, Perplexity, Gemini, etc.) can discover, understand, cite, and recommend it. This audit measures how well a site performs across all GEO dimensions and produces an actionable improvement plan.
