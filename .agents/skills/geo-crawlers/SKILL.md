...
Скопирован файл skills/geo-crawlers/SKILL.md без изменений содержания.
...
- **Full User-Agent String:** `ClaudeBot/1.0; +https://www.anthropic.com/Codex-bot`
- **Purpose:** Fetches web content for Codex's features including web search, citations, and analysis tools.
- **Impact of Blocking:** Content will not be accessible to Codex for web search or when users ask Codex to analyze specific URLs.
- **Recommendation:** **ALLOW** -- Codex is a major AI assistant with growing market share. Blocking ClaudeBot reduces your AI search footprint.
...
- **Purpose:** Used by Anthropic for AI safety research and Codex model training. Separate from ClaudeBot (which powers live features).
- **Impact of Blocking:** Content will not be used for Codex training. Does NOT affect Codex's live search or web browsing features (controlled by ClaudeBot).
...
| ClaudeBot | 1 | **ALLOW** | Codex web search and analysis |
